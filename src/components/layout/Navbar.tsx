  ...
  return <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'glass shadow-sm py-3' : 'bg-transparent py-5'}`}>
      <div className="container mx-auto px-6 lg:px-8">
        <div className="flex items-center justify-between">
          <Link to="/" className="flex items-center space-x-2">
            ...
            <span className="text-xl text-purple-700 font-bold rounded-text" style={{ 
              fontFamily: "'Poppins', system-ui, sans-serif", 
              letterSpacing: "0", 
              fontWeight: "600",
              fontSize: "1.35rem"
            }}>BizSimpl Startup</span>
          </Link>
          ...
          <nav className="hidden md:flex items-center justify-center flex-grow mx-auto">
            <div className="flex items-center space-x-1">
              ...
              {navLinks.map(link => <Link key={link.name} to={link.href} className="px-3 py-2 text-sm font-medium text-purple-700 hover:text-purple-800 transition-colors rounded-md">
                  {link.name}
                </Link>)}
            </div>
          </nav>
          ...
          <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="inline-flex md:hidden items-center justify-center p-2 rounded-md text-gray-700 hover:text-purple-700 hover:bg-gray-100 transition-colors">
      ...
      <div className={`md:hidden transition-all duration-300 overflow-hidden ${isMobileMenuOpen ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="px-6 pt-2 pb-4 space-y-1 bg-white border-t border-gray-100">
          <div className="block">
            <Collapsible>
              ...
              <CollapsibleContent className="pl-4 space-y-1 mt-1">
                <Link to="/private-limited" className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-gray-700 hover:text-purple-700 hover:bg-gray-50 rounded-md" onClick={() => setIsMobileMenuOpen(false)}>
                  <Building className="h-4 w-4" />
                  <span>Private Limited Company</span>
                </Link>
                <Link to="/llp" className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-gray-700 hover:text-purple-700 hover:bg-gray-50 rounded-md" onClick={() => setIsMobileMenuOpen(false)}>
                  <Briefcase className="h-4 w-4" />
                  <span>Limited Liability Partnership</span>
                </Link>
                <Link to="/one-person-company" className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-gray-700 hover:text-purple-700 hover:bg-gray-50 rounded-md" onClick={() => setIsMobileMenuOpen(false)}>
          ...
          {navLinks.map(link => <Link key={link.name} to={link.href} className="block px-3 py-2 text-base font-medium text-purple-700 hover:text-purple-800 hover:bg-gray-50 rounded-md" onClick={() => setIsMobileMenuOpen(false)}>
              {link.name}
            </Link>)}
          
          <div className="pt-2">
            <Link to="/registration" className="block w-full" onClick={() => setIsMobileMenuOpen(false)}>
